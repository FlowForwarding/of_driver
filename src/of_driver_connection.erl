    ...
    case ofp_parser:parse(Parser, Data) of
        {ok, NewParser, Messages} ->
            NewState = lists:foldl(
                            fun(Message, AccState) ->
                                handle_message(Message, AccState)
                            end, State, Messages),
...
handle_message(#ofp_message{
                    version = Version,
                    type = features_reply,
                    body = Body}, State) when Version =:= 3 ->
    [DatapathID, DatapathMac] =
                            of_driver_utils:get_datapath_info(Version, Body),
...
handle_message(#ofp_message{
    ...
    NewState=
        case of_driver_db:lookup_datapath_id({DatapathID, DatapathMac}) of
            [] when AuxID =:= 0 ->
                {ok, ChannelPID} = of_driver_channel_sup:start_child(
                                                    {DatapathID, DatapathMac}),
                % io:format("[~p] Created channel ~p~n",[?MODULE,ChannelPID]),
                of_driver_db:insert_datapath_id(
                                {DatapathID, DatapathMac}, self(), ChannelPID),
                State#?STATE{conn_role = main};
            [Entry] when AuxID =/= 0 ->
                % add aux entry
                % io:format("[~p] Adding Aux entry AuxID:~p~n",[?MODULE,AuxID]),
                of_driver_db:add_aux_id(Entry, {DatapathID, DatapathMac},
                                                            [AuxID, self()]),
...
handle_message(#ofp_message{ version = _Version, type = _Type, body = _Body } = _Msg,State) ->
    %% io:format("[~p] Unknown MSG ~p Not doing anything~n",[?MODULE,Body]),
    State.
