...
revoke_ipaddr(IpAddr) -> 
    %% TODO: Closes any existing connections from IpAddr and calls appropriate callbacks.  Does nothing if IpAddr was not in the allowed list.    
...
-spec send(Connection :: term(), Msg :: #ofp_message{}) -> ok | {error, Reason :: term()}.
send(_Connection, _Msg = #ofp_message{}) -> 
    %% Connection ! Msg ...
    ok.

-spec sync_send(Connection :: term(), Msg :: #ofp_message{}) -> {ok, Reply :: #ofp_message{} | noreply} | {error, Reason :: term()}.
